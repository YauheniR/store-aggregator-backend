...
class CategoriesApiTestCase(UserRegistration):
    ...
    def setUp(self) -> None:
        super().setUp()
        self.data = CategoryModel.objects.bulk_create([CategoryModel(name='Category 1'),
    ...
    def test_get_category_call_200_ok(self):
        response = self.client.get(reverse('category_detail', args=(self.detail_data.id,)))
    ...
    def test_put_category_call_200_ok(self):
        response = self.client.put(reverse('category_detail', args=(self.detail_data.id,)), {'name': 'New Name'})
    ...
    def test_patch_category_call_200_ok(self):
        response = self.client.patch(reverse('category_detail', args=(self.detail_data.id,)), {'name': 'Some Name'})
    ...
    def test_delete_category_call_204_no_content(self):
        response = self.client.delete(reverse('category_detail', args=(self.detail_data.id,)))
...
class ProviderCategoriesApiTestCase(UserRegistration):
    ...
    def setUp(self) -> None:
        ...
        ProviderCategoryModel.objects.create(url=self.url_provider_category,
                                             category=self.category,
                                             provider=self.provider, )

        self.detail_data = ProviderCategoryModel.objects.get(url=self.url_provider_category)

    def test_get_provider_category_call_200_ok(self):
        response = self.client.get(reverse('categories_providers_detail', args=(self.category.id, self.detail_data.id)))
    ...
    def test_put_provider_category_call_200_ok(self):
        ...
        response = self.client.put(
            reverse('categories_providers_detail', args=(self.category.id, self.detail_data.id,)),
            {
                "url": put_url,
                "provider": self.provider.id,
                "category": self.category.id
    ...
    def test_delete_provider_category_call_204_no_content(self):
        response = self.client.delete(reverse('categories_providers_detail', args=(self.category.id, self.detail_data.id,)))
